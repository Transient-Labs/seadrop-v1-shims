...
contract ERC721SeaDropCloneFactoryTest is Test {
    ...
    function testClone1() public {
        ...
        assertEq(token.tokenURI(1), token.baseURI(), "tokenURI just the baseURI");

        token.setBaseURI("https://example.com/");
        assertEq(token.tokenURI(1), string(abi.encodePacked(token.baseURI(), "1")), "tokenURI the baseURI + tokenID");
    }
