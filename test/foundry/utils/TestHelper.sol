...
contract TestHelper is Test, SeaDropErrorsAndEvents {
    ...
    function _encodeSignature2098(bytes32 r, bytes32 s, uint8 v) internal pure returns (bytes memory) {
        uint256 yParity;
        if (v == 27) {
            yParity = 0;
        } else {
    ...
    function _deriveDomainSeparator() internal view returns (bytes32) {
        // prettier-ignore
        return
            keccak256(abi.encode(_EIP_712_DOMAIN_TYPEHASH, _NAME_HASH, _VERSION_HASH, block.chainid, address(seadrop)));
    }
}
